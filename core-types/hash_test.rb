...
class HashTest < Minitest::Test
  ...
  def test_access
    ...
    assert_equal %w[Alice Bob], ages[16]
  ...
  def test_access_with_symbols
    ...
    assert_equal %w[Alice Bob], ages[:siblings]
  ...
  def test_assignment
    ...
    assert_equal %w[aardvark apple apricot], stuff["a"]
    assert_equal "brandy", stuff["b"]
    assert_equal %w[cook caramel\ candy], stuff["c"]
  end
  ...
  def test_assignment_with_symbols
    ...
    expected_dahlias = %w[Crichton\ Honey Whimsey Kasasagi Jitterbug]
    
    assert_equal expected_dahlias, plants[:dahlia]
    assert_equal %w[Black\ Prince Madame\ Butterfly], plants[:snapdragon]
    assert_equal %w[Majorette\ Mix], plants[:hollyhock]
  end

  def test_keys
    assert_equal [], {}.keys
    assert_equal [1, 2], {1 => "one", 2 => "two"}.keys
    assert_equal %w[a banana], {"a" => "apple", "banana" => "b"}.keys
  end
  ...
  def test_values
    assert_equal [], {}.values
    assert_equal %w[one two], {1 => "one", 2 => "two"}.values
    assert_equal %w[apple b], {"a" => "apple", "banana" => "b"}.values
  end
  
  def test_values_with_symbols
    assert_equal %w[Paris Vilnius], {france: "Paris", lithuania: "Vilnius"}.values
  ...
  def test_update_with_symbols
    ...
    expected_favorite_restaurants = {"cultura": "Latin American", "wallys": "Wisconsin treats"}
    expected_name_lengths = {maeby: 5, selena: 6}
    expected_name_lengths_2 = {maeby: 5, selena: 6, "maeby": 5}
    expected_planet_rings = {saturn: 7, jupiter: 3, uranus: 13, neptune: 6}
